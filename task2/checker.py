...
def get_depth(circuit):
    ...
    def get_depth_at_node(node):
        ...
        if node not in circuit.gates:
            return 0
        depths[node] = 1 + \
            max(map(get_depth_at_node, circuit.gates[node].inputs))
        return depths[node]
...
def evaluate_circuit(circuit, input_values, is_vectorized=False):
    ...
    def evaluate_at_node(node):
        ...
        evaluations[node] = operations[gate.opcode](
            *map(evaluate_at_node, gate.inputs))
        return evaluations[node]
...
def check_3_2_trick(circuit, n):
    ...
    print("a = {}, b = {}, c = {}, a+b+c = {}\nx = {}, y = {}, x+y = {}\n".format(a,
          b, c, addbinary(a, b, c), x, y, addbinary(x, y)))
...
def check(reply, n):
    ...
    return all(check_3_2_trick(c, n) for _ in range(n_random_checks)) and get_depth(c) <= 5 and get_size(c) <= 12*n + 1
